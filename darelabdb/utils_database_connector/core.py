...
import pandas as pd
from darelabdb.utils_configs.databases import DatabaseConfig, settings
...
class Database:
    ...
    def _parse_query(self, query: str, limit: int, order_by_rand=False, only_read=True):
        ...
        if (
            only_read
            and type(pars) != expressions.Select
            and type(pars) != expressions.Union
            and type(pars) != expressions.Intersect
            and type(pars) != expressions.Except
        ):
            raise ValueError(
