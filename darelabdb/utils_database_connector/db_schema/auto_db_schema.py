...
def insert_types(db: Database | DatabaseSqlite, schema: list) -> None:
...
def insert_primary_keys(db: Database | DatabaseSqlite, schema: list) -> None:
...
def insert_foreign_keys(db: Database | DatabaseSqlite, schema: list) -> None:
...
if __name__ == "__main__":
    ...
    # dbb = Database("fc4eosc", specific_schema="fc4eosc_subset")
    dbb = DatabaseSqlite(
        "/home/katso/.cache/darelabdb/datasets/BIRD/dev_databases/california_schools/california_schools.sqlite"
    )
    schemaa = obtain_schema_from_db(dbb, infer_foreign_keys=True)
    pp(schemaa)
