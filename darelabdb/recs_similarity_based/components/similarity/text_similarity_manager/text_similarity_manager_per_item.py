...
class TextSimilarityManagerPerItem(SimilarityManager):
    """
    The TextSimilarityManagerPerItem class is responsible for the creation and caching of the items' text similarity.
    For each item the similarity manager will store the pairwise similarities with a given subset of the corpus items.
    """
    ...
    def initialise(
        ...
        similarities_cache: Dict[item_id_type, List[tuple[item_id_type, float]]] = None,
    ) -> None:
        """
        Checks if the text similarities already exist.
        If not it creates and stores the similarities between given items (Recommended if available processes <= 2).

        Args:
            embeddings (DataFrame): A dataframe with the ids and the embeddings of items.
            similarities_cache (Dict[item_id_type, List[tuple[item_id_type, float]]]): A dictionary with precalculated
    ...
    def update(
        ...
        """
        Creates and stores the pairwise text similarity of the first item embedding with the rest of the
        given embeddings (Recommended if available processes <= 2).
    ...
    def update_batch(
        ...
        similarities = calculate_similarities_of_embeddings_batch(
            embeddings_list, sdr_similarity_calculation_per_item, proc_numb
        )
        start = time.time()
